...
select asig.id_asignatura, asig.asignatura, prof.id_profesor, prof.nombre, concat(prof.apellido1, ' ', prof.apellido2) apellidos, truncate(avg(notas.nota),2) medida_nota
from profesores prof
...
select count(al.id_alumno) cantidad, al.ciudad
from alumnos al
join notas on al.id_alumno = notas.fk_alumno
where notas.nota >= (select avg(notas.nota) media_nota from notas where notas.nota is not null)
group by al.ciudad;

-- 17 Notas de las asignaturas de cada uno de los alumnos comparada con la nota media de la asignatura
select al.id_alumno, al.nombre, asig.asignatura, notas1.nota, (select avg(notas2.nota) from notas notas2 where notas2.fk_asignatura = notas1.fk_asignatura) as media_nota
from notas notas1
join alumnos al on al.id_alumno = notas1.fk_alumno
join asignaturas asig on asig.id_asignatura = notas1.fk_asignatura;
